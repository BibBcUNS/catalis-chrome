...
// -----------------------------------------------------------------------------
{
	switch (type) {
		case "ISBN" :
		    //alert("ISBN -- Ignore este mensaje y disculpe");
			var number_error = true;
			if ( box.value == "" ) {
				number_error = false;
				//alert("No se ingresó ISBN, todo bien -- Ignore este mensaje y disculpe");
				...
				var matches = isbn_pattern.exec(box.value);
				if (matches) {
				    var myValue = matches[0];
				    var myisbn = ISBN.parse(myValue);
				    number_error = !myisbn.isIsbn10() && !myisbn.isIsbn13();
				    alert("myValue: " + myValue + "\nisIsbn10: " + myisbn.isIsbn10() + "\nisIsbn13: " + myisbn.isIsbn13() + "\nnumber_error: " + number_error);
				}
		...
		case "ISSN" :
			...
			if ( box.value == "" ) {
				number_error = false;
					...
					number_error = !isNumberValid(myValue,type);
			} else {
				number_error = true;
			}
			break;
	...
	if ( number_error ) {   // tenemos un error
