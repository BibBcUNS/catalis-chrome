...
//--------------------------------------------------------
{
	...
	var n1 = ej1["t"].replace(/\D/g,"");
	var n2 = ej2["t"].replace(/\D/g,"");
	return n1 - n2;
}
...
// -----------------------------------------------------------------------------
{
	...
	// Ejemplares
	// Solo mostramos más de una signatura si difieren en: prefijo, clase, o libristica.
	if ( ejemplares.length > 0 ) {
		...
		var prevKey = ejemplares[0]["k"].concat(ejemplares[0]["h"]).concat(ejemplares[0]["i"]); // ???
		for ( var i=1; i < ejemplares.length; i++ ) {
			var currentKey = ejemplares[i]["k"].concat(ejemplares[i]["h"]).concat(ejemplares[i]["i"]);
